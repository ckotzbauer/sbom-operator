...
func initTargets(k8s *kubernetes.KubeClient) []target.Target {
	...
	for _, ta := range internal.OperatorConfig.Targets {
		...
		if ta == "git" {
			...
			k8sCommonPrefix := internal.OperatorConfig.KubernetesCommonPrefix
			...
			t := dtrack.NewDependencyTrackTarget(baseUrl, apiKey, podLabelTagMatcher, caCertFile, clientCertFile, clientKeyFile, k8sClusterId, k8sCommonPrefix, defaultParentProject, parentProjectAnnotationKey, projectNameAnnotationKey, useShortName)
			err = t.ValidateConfig()
			targets = append(targets, t)
		} else if ta == "oci" {
